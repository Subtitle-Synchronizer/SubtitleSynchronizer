...
def synchronize(video_file, subtitle_file, output_file, verbose=False, \
    ...
    **kwargs):
    ...
    # load model
    print('model fie: ', model_file)
    trained_model = model.load(model_file)
    print('bias: ', trained_model[1])
    ...
    np.save('features_file_X.npy', features_x)
    np.save('features_file_Y.npy', shifted_y)
    ...
    classification_report = metrics.classification_report(shifted_y, y_converted_scores)
    print("Classification report: \n", classification_report)
    confusion_matrix = metrics.confusion_matrix(shifted_y, y_converted_scores)
    print("Confusion matrix: \n", confusion_matrix)

    print("Accuracy Score: ", accuracy_score(shifted_y, y_converted_scores))
    print("F1 Score: ", f1_score(shifted_y, y_converted_scores, average='weighted'))
    print("Precision Score: ", precision_score(shifted_y, y_converted_scores, average='weighted'))
    print("Recall Score: ", recall_score(shifted_y, y_converted_scores, average='weighted'))
...
def cli(packaged_model=False):
    ...
    #p.add_argument('video_file', default='Deadpool2.mp4', help='Input video file')
    #p.add_argument('subtitle_file', default='Deadpool2.srt', help='Input SRT subtitle file')
    #p.add_argument('output_file', default='Deadpool2_Output.srt', help='Output (auto-synchronized) SRT subtitle file')

    # Make model file an argument only in the non-packaged version
    print('packaged_model: ', packaged_model)
    ...
    subtitle_file='Deadpool2.srt'
    output_file = 'Deadpool2_Output.srt'

    video_file = 'GardenofEvil.mp4'
    subtitle_file = 'GardenofEvil_10_2.srt'
    output_file = 'GardenofEvil_10_2_Output_autosubsync_with_librosa.srt'
    success = synchronize(video_file, subtitle_file, output_file,\
