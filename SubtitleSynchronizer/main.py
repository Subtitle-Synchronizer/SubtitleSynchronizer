...
def synchronize(video_file, subtitle_file, output_file, verbose=False, \
    ...
    **kwargs):
    ...
    # video file should not be longer than 10 min
    video_duration = preprocessing.get_video_length(video_file)
    if video_duration > 600:
        # Can not process a video file which is longer than 10 mins.
        sys.stderr.write('Error: Cannot process more than 10min longer video. Video duration: %d sec\n' % video_duration)
        sys.exit(1)

    # argument parsing
    if model_file is None:
        from pkg_resources import resource_filename
        model_file = resource_filename(__name__, '../subtitlesynchronizer.trained.model.bin')

    print('Input video file: ', video_file)
    print('Input SRT file: ', subtitle_file)
    print('Output SRT file: ', output_file)
...
def cli(packaged_model=False):
    ...
    if not packaged_model:
        p.add_argument('--model_file', default='subtitlesynchronizer.trained.model.bin')
